...
src/
  ...
  tools/
    _shared.ts      response helpers + resolveUserId (arg → ALLTRAILS_USER_ID → /api/alltrails/me users[0].id) + the compact projections, applied BY DEFAULT via the fleet `view` param (summarizeTrail/TrailDetail/Review/Photo/SearchResult/FeedItem/ListItem/Location + their loose schemas + ALLTRAILS_VIEWS)
    trails.ts       get_trail, get_trail_reviews, get_trail_photos, get_trail_weather, get_trail_gpx
    explore.ts      search, resolve_location (suggestions → place records: kind/coords/slug)
    user.ts         get_profile, list_user_lists, get_list_items (GET /lists/{id}/items — sparse trailId refs), list_completed_trails, get_activity_feed (feed?: local|timeline|personal follows the actual feed; omitted → directory)
...
## Response shape (`view`) — and why it is not called `detail`

Six read tools take the fleet `view` parameter (`ALLTRAILS_VIEWS` in `tools/_shared.ts`, on `@chrischall/mcp-utils`' vocabulary): `alltrails_search`, `alltrails_get_trail`, `alltrails_get_trail_reviews`, `alltrails_get_trail_photos`, `alltrails_get_list_items`, `alltrails_get_activity_feed`. Two rungs, `compact` (the DEFAULT) and `full`. No `raw`: `full` already returns the untouched upstream payload here, so a third rung would silently alias to another.

The `summarize*` projections predate the parameter — they were opt-in behind `compact: false`, with the tool descriptions asking the caller to "Set compact=true (strongly recommended)". An efficiency that has to be requested is one that usually is not, and the caller paying for it is the one least able to know the slim rung existed. Defaulting it on is safe because of a property that also predates the change: every compact branch parses, checks the array is where it expects, and **falls through to the raw payload when it is not** — never an empty or half-filled projection.

**`view` is not `detail`, and this repo is the reason the fleet parameter is named `view` at all.** `alltrails_get_trail`'s `detail` (`basic`/`medium`/`offline`) is an **upstream query param** — it decides what AllTrails sends *us*. `view` decides what we hand the caller. They compose rather than overlap: route geometry needs `detail: "offline"` to be fetched AND `view: "full"` to survive the projection. Do not "unify" the two.

**`view` must never reach AllTrails.** Every handler here takes `async (args)` and builds its request body field by field (`{ query: args.query, limit, recordTypesToReturn }`), which is what keeps `args.view` out of the wire — `client.request` is never handed the argument object, and nothing spreads it (`{ ...args }`). Two sibling repos shipped that leak. `tests/tools/view-leak.test.ts` asserts the property directly rather than trusting the convention to hold.

One projection is **not** a subset of `full`: `summarizePhoto` DERIVES and signs the image `url` (AllTrails' photo record carries no image URL — see the API notes), so `view: "full"` genuinely lacks a field `compact` has. The tool's `view` note says so.
...
- **⚠️ `GET /api/alltrails/locations/{states|countries}/{id}/trails` was RETIRED** (confirmed 2026-07-08): it returns 400 for every id/param/path variant (region id 34, Algolia id 38, the documented country 313; with/without `page`/`per_page`/`algolia_formatted`; `v2` path; slug form), and the live state/country pages now make **zero** `/api/alltrails/*` XHR — the trail list is server-rendered into the page as Next.js RSC flight chunks (`self.__next_f`), with the rich fields (rating/length/difficulty) stripped from the markup. The former `alltrails_list_trails_by_state`/`by_country` tools rode this endpoint and were **removed** rather than repointed to a lossy SSR scrape; use `alltrails_search` (Algolia-backed, rich records) to find trails in/near a place. CI mocks the client, so the breakage never surfaced in tests.
- Response shapes below were captured live 2026-07-02 (via an in-browser fetchproxy bridge probe — Node replays of a captured cookie were 403'd by DataDome that day, but same-origin in-tab fetches sail through):
  ...
  - **`GET /api/alltrails/lists/{id}/items`** (captured 2026-07-08) → `{ listItems: [{ id, listId, type, order, notes, trailId, metadata.created }], meta: { items } }`. Items are **sparse references** — only a `trailId` (hydrate via `get_trail`) plus the curator's `order`/`notes`; no trail details are inlined. `alltrails_get_list_items` projects + sorts these. The `.../users/{id}/lists` envelope carries no link to this endpoint (path is a fixed `/lists/{id}/items`).
