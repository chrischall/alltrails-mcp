...
src/
  ...
  tools/
    ...
    explore.ts      search, resolve_location (suggestions → place records: kind/coords/slug), list_trails_by_state, list_trails_by_country
...
- **⚠️ `GET /api/alltrails/locations/{states|countries}/{id}/trails` (the `list_trails_by_state`/`by_country` backing endpoint) now returns 400** for every id/param/path variant probed 2026-07-08 (region id 34, Algolia id 38, the documented country 313; with/without `page`/`per_page`/`algolia_formatted`; `v2` path; slug form). The endpoint family appears gone — those two tools are non-functional against the live API (tracked in the repo issues). CI mocks the client so it didn't surface.
