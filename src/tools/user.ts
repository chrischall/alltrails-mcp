...
export function registerUserTools(server: McpServer, client: AllTrailsClient): void {
  ...
  server.registerTool(
    ...
    async (args) => {
      ...
      const data = await client.request(
        'GET',
        `/api/alltrails/users/${encodeURIComponent(userId)}/lists`,
      );
      return jsonResponse(data);
    },
  );
  ...
  server.registerTool(
    ...
    async (args) => {
      const raw = await client.request(
        'GET',
        `/api/alltrails/lists/${encodeURIComponent(args.listId)}/items`,
      );
  ...
  server.registerTool(
    ...
    async (args) => {
      ...
      const data = await client.request(
        'GET',
        `/api/alltrails/users/${encodeURIComponent(userId)}/trails/completed`,
      );
      return jsonResponse(data);
    },
  );
    ...
    'alltrails_get_activity_feed',
    {
      ...
      inputSchema: z.object({
        ...
        feed: z
          ...
          .describe(
            'Which feed to read: local (nearby activity), timeline (people you follow), personal (own posts). Omit to list the available feeds.',
          )
          .optional(),
        maxItems: z
          .number()
          .int()
          .positive()
          .describe('Max items per page (server-side)')
          .optional(),
        cursor: z
          .string()
          .describe('Opaque nextCursor from a previous page, for pagination')
          .optional(),
        view: viewParam(ALLTRAILS_VIEWS, {
          note: 'compact returns { count, hasNextPage, nextCursor, items: [{ type, timestamp, description, user, trail, activity, review }] } — and the no-feed directory as { name, displayName } per feed; "full" returns AllTrails\' whole records.',
        }),
      }),
    },
    async (args) => {
